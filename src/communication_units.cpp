...
/*
@brief: handles the messages recieved from sensor units to return back to the
raspberry pi
...
int handleMSG_CU(const def_message_struct &msgRecv) {
  ...
  if (strncmp(msgRecv.message, sens_unit_response[0],
              strlen(sens_unit_response[0])) == 0) {
    ...
    if (status_index >= 0 && status_index < 3) {
      strncat(returnVal, status_strings[status_index],
              sizeof(returnVal) - strlen(returnVal) - 1);
    } else {
      strncat(returnVal, "INVALID_STATUS",
              sizeof(returnVal) - strlen(returnVal) - 1);
    }
  } else if (strncmp(msgRecv.message, sens_unit_response[1],
                     strlen(sens_unit_response[1])) == 0) {
    for (int i = 0; i < msgRecv.numValues; i++) {
      ...
      strncat(returnVal, pyStrSeper, sizeof(returnVal) - strlen(returnVal) - 1);
      strncat(returnVal, sens_unit_strings[(int)msgRecv.values[i]],
              sizeof(returnVal) - strlen(returnVal) - 1);
    }
  } else {
    for (int i = 0; i < msgRecv.numValues; i++) {
    ...
    if (msgRecv.value[0] != '\0') {
      strncat(returnVal, pyStrSeper, sizeof(returnVal) - strlen(returnVal) - 1);
      strncat(returnVal, msgRecv.value,
              sizeof(returnVal) - strlen(returnVal) - 1);
    }
  }
  char SUStr[2];
...
void respondPiRequest(const char *str) {
  ...
  if (keywords[0] != NULL && keywords[1] != NULL &&
      strncmp(keywords[0], "INIT", strlen("INIT")) == 0 &&
      strncmp(keywords[1], "PI", strlen("PI")) == 0) {
    int i;
    int j;
    char initStr[MAXPYSTRINGLEN];
    initStr[0] = '\0';
    int strLength = 0;
    // When defining a sensor we pass the header 'SENSOR', the pyStringSeper and
    // all of the commands and responses to the commands are seperated by commas
    // To allow raspberry pi to sort the values
    // Excluding '
    // The order goes
    // SENSOR|NAME|'commands,seperated,by,commas,'|'return,strings,seperated,by,commas,|
    for (i = 0; i < NUM_OF_SENSORS; i++) {
      ...
      while (sensors[i].commands[j] != NULL) {
        if (strlen(sensors[i].commands[j]) <
            sizeof(initStr) - strlen(initStr) - 1) {
          strncat(initStr, sensors[i].commands[j],
                  sizeof(initStr) - strlen(initStr) - 1);
          strncat(initStr, ",", sizeof(initStr) - strlen(initStr) - 1);
        } else {
          stageForReturn(
              "Increase string buffer size in INIT|PI functionality");
          return;
        }
        j++;
      }
      ...
      while (sensors[i].responses[j] != NULL) {
        if (strlen(sensors[i].responses[j]) <
            sizeof(initStr) - strlen(initStr) - 1) {
          strncat(initStr, sensors[i].responses[j],
                  sizeof(initStr) - strlen(initStr) - 1);
          strncat(initStr, ",", sizeof(initStr) - strlen(initStr) - 1);
        } else {
          stageForReturn(
  ...
  if (keywords[0] != NULL && keywords[1] != NULL &&
      strncmp(keywords[0], "PULL", strlen(keywords[0])) == 0 &&
      strncmp(keywords[1], "ALL", strlen("ALL")) == 0) {
    ...
    for (i = 0; i < com_unit_ptr->numOfSU; i++) {
      j = 0;
      while (sensors[BASE_SENS_UNIT].commands[j] != NULL) {
        if (strncmp(sensors[BASE_SENS_UNIT].commands[j], "PULL",
                    strlen("PULL")) == 0) {
          msg.strlen = 0;
          msg.message[0] = '\0';
          msg.strlen = snprintf(msg.message, sizeof(msg.message), "%s",
                                sensors[BASE_SENS_UNIT].commands[i]);
          sendMessage(com_unit_ptr->SU_ADDR[i], (uint8_t *)&msg,
      ...
      for (j = 0; j < com_unit_ptr->SU_NUM_MODULES[i]; j++) {
        sensor_type sensor = com_unit_ptr->SU_AVLBL_MODULES[i][j];
        k = 0;
        while (sensors[sensor].commands[k] != NULL) {
          if (strncmp(sensors[sensor].commands[k], "PULL", strlen("PULL")) ==
              0) {
            msg.strlen = 0;
            msg.message[0] = '\0';
            msg.strlen = snprintf(msg.message, sizeof(msg.message), "%s",
    ...
    for (i = 0; i < NUM_OF_SENSORS; i++) {
      j = 0;
      while (sensors[i].commands[j] != NULL) {
        if (strncmp(sensors[i].commands[j], keywords[0],
                    sizeof(sensors[i].commands[j])) == 0) {
          sensor_found = true;
        }
        j++;
      }
    ...
    // If we pass a command and are pulling from all available units then send
    // them to every unit which contains the sensor and command which was found
      ...
      for (i = 0; i < com_unit_ptr->numOfSU; i++) {
        for (j = 0; j < com_unit_ptr->SU_NUM_MODULES[i]; i++) {
          if (sensor == com_unit_ptr->SU_AVLBL_MODULES[i][j]) {
            msg.message[0] = '\0';
            msg.strlen =
                snprintf(msg.message, sizeof(msg.message), "%s", keywords[0]);
            sendMessage(com_unit_ptr->SU_ADDR[i], (uint8_t *)&msg.message,
                        sizeof(msg));
          }
        }
      }
               ...
               strncmp(keywords[1], "IND", strlen("IND")) == 0 &&
               keywords[2] != NULL) { // Else we pass an index for a sensor unit
                                      // to pull from
      int ind = 0;
      int pow = 1;
